...
struct fixture {
    ...
    fixture(ggml_type type, int64_t experts, bool mixed = false, int64_t embd = 32, int64_t ff = 32,
            bool scaled = false, bool fused = false, bool random_weights = false) {
        ctx = ggml_init({ 8 * ggml_tensor_overhead(), nullptr, true });
        check(ctx != nullptr, "fixture context");
        for (size_t p = 0; p < 3; ++p) {
            tensors[p] = ggml_new_tensor_3d(ctx, mixed && p == 2 ? GGML_TYPE_Q8_0 : type,
        ...
        for (size_t p = 0; p < 3; ++p) {
            ...
            const size_t written = ggml_quantize_chunk(t->type, values.data(), bytes.data(), 0, t->ne[1] * experts, t->ne[0], nullptr);
...
static llama_moe_reference_result ordinary(const llama_moe_host_bank & bank,
    ...
    ggml_set_input(w);
    auto * gate = ggml_mul_mat_id(ctx, const_cast<ggml_tensor *>(bank.tensor(0)), x, route);
    auto * up = ggml_mul_mat_id(ctx, const_cast<ggml_tensor *>(bank.tensor(1)), x, route);
    auto * act = gelu ? ggml_geglu_split(ctx, gate, up) : ggml_swiglu_split(ctx, gate, up);
    auto * experts = ggml_mul_mat_id(ctx, const_cast<ggml_tensor *>(bank.tensor(2)), act, route);
...
static void test_cached_hit_priority() {
    ...
    auto incompatible = std::make_shared<fixture>(GGML_TYPE_F16, 7);
