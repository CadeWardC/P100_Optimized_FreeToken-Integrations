...
public:
    ...
    host_bank(std::shared_ptr<const void> owner,
...
struct reference_result : execution_stats {
...
struct cache_metrics {
    ...
    size_t retained_compute_bytes = 0;
    uint64_t workspace_builds = 0, workspace_reuses = 0;
    uint64_t graph_captures = 0, graph_replays = 0;
};
...
struct executor_options {
    ...
    bool pipeline = false;
    bool fast = false;
    bool device_tensors = true;
};

// Serialized executor. Device execution must be called outside a backend callback.
class cached_executor {
public:
    cached_executor(const std::vector<std::shared_ptr<host_bank>> & banks, size_t budget,
            executor_options options = {});
    ~cached_executor();
    cached_executor(const cached_executor &) = delete;
    cached_executor & operator=(const cached_executor &) = delete;
    reference_result execute(size_t layer, const std::vector<float> & input, size_t n_tokens, size_t top_k,
            const std::vector<int32_t> & ids, const std::vector<float> & weights, int n_threads,
            freetoken::activation activation);
    // The caller drains producer backends before entry. This call drains before returning.
    bool supports_device(size_t n_tokens) const;
    ggml_backend_dev_t device() const;
    execution_stats execute_device(size_t layer, const ggml_tensor * input, const ggml_tensor * ids,
            const ggml_tensor * weights, ggml_tensor * dst, freetoken::activation activation);
    cache_metrics metrics() const;
...
size_t min_tile_bytes(const host_bank & bank, ggml_backend_buffer_type_t buft = nullptr);
...
struct cpu_op {
    std::shared_ptr<host_bank> bank;
    std::shared_ptr<cached_executor> executor;
